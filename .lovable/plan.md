...
Hoy el formulario de `/login` (modo registro) sólo pide nombre, email y contraseña, aunque el backend ya acepta `telefono` y `direccion` de forma opcional y crea el perfil de cliente en la misma transacción.

- Frontend (`src/routes/_store.login.tsx`): agregar los campos **Teléfono** y **Dirección** en modo registro, con validación (teléfono: requerido, sólo dígitos/espacios/`+`/`-`, máx. 50; dirección: requerida, máx. 250) y enviarlos en `register`.
...
- En **alta**: nombre, email, contraseña, teléfono y dirección (mismos campos y validaciones que el registro público). Se envía a `POST /clientes`, que ya crea usuario + perfil.
- En **edición**: nombre, teléfono y dirección editables; email visible pero de sólo lectura (cambiar el email de acceso es otra operación), y sin campo de contraseña.
...
## Nota sobre direcciones múltiples

No se crea `ClienteDirecciones` (1-M) ahora. El diseño elegido deja el camino abierto: el pedido guarda su propia dirección de envío, así que más adelante alcanza con agregar la tabla y una lista de direcciones seleccionables sin romper lo existente.
